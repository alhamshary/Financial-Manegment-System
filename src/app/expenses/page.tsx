...
import { MoreHorizontal, PlusCircle, Loader2, Calendar as CalendarIcon, DollarSign } from "lucide-react";
...
export default function ExpensesPage() {
    ...
    const handleFormSubmit = async (values: ExpenseFormValues) => {
        ...
        if (editingExpense) {
            ...
            const { error } = await supabase.from('expenses').insert(expenseData as TablesInsert<'expenses'>);
            if (error) {
        ...
        <AppLayout>
            ...
            <Card className="mt-6">
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                           <CardTitle>{user?.role === 'employee' ? 'مصاريفك اليوم' : 'جميع المصاريف'}</CardTitle>
                           <CardDescription>
                                {user?.role === 'employee' ? 'قائمة بجميع المصاريف التي سجلتها اليوم.' : 'قائمة بجميع المصاريف بناءً على الفلاتر المحددة.'}
                           </CardDescription>
                        </div>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                {(user?.role === 'admin' || user?.role === 'manager') && <TableHead>الموظف</TableHead>}
                            ...
                            {loading ? (
                                ...
                                expenses.map((expense) => (
                                    <TableRow key={expense.id}>
                                        ...
                                        {(user?.role === 'admin' || user?.role === 'manager') && <TableCell><Badge variant="outline">{expense.users?.name || 'غير معروف'}</Badge></TableCell>}
                                            ...
                                            {canEditOrDelete(expense) ? (
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end">
                                                        ...
                                                        <DropdownMenuItem onClick={() => handleOpenEditDialog(expense)}>تعديل</DropdownMenuItem>
                                                        <DropdownMenuItem onClick={() => setDeletingExpense(expense)} className="text-destructive">حذف</DropdownMenuItem>
                                                    </DropdownMenuContent>
                                                </DropdownMenu>
                                            ) : null}
                                        </TableCell>
            ...
            <Dialog open={isAddOrEditDialogOpen} onOpenChange={(isOpen) => !isOpen && handleCloseDialogs()}>
                <DialogContent className="sm:max-w-[425px]" onInteractOutside={handleCloseDialogs}>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {editingExpense ? "تحديث تفاصيل هذا المصروف." : "املأ تفاصيل المصروف الجديد."}
                        </DialogDescription>
                    </DialogHeader>
                    <div className="py-4">
                        <ExpenseForm
            ...
            <AlertDialog open={!!deletingExpense} onOpenChange={() => setDeletingExpense(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>هل أنت متأكد أنك تريد حذف هذا المصروف؟</AlertDialogTitle>
                        <AlertDialogDescription>
                            لا يمكن التراجع عن هذا الإجراء. سيؤدي هذا إلى حذف المصروف "{deletingExpense?.name}" نهائيًا.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
